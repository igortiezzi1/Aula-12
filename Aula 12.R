...
> plot(emprego, main="Pessoa Empregadas no Brasil",  lab="Qte de Pessoas Empregadas-milhões", lab="Ano"
       + 
         
         > plot(emprego, main="Pessoa Empregadas no Brasil",  lab="Qte de Pessoas Empregadas-milhões", lab="Ano")
       Error in plot.window(xlim, ylim, log, ...) : 
         parâmetro gráfico "lab" tem o comprimento errado
       In addition: Warning message:
         In plot.window(xlim, ylim, log, ...) : NAs introduced by coercion
       > plot(emprego, main="Pessoa Empregadas no Brasil",  ylab="Qte de Pessoas Empregadas-milhões", xlab="Ano")
       > acf(emprego)
       ...
       > arima220 <- arima(emprego, c(1,2,2))
       > arima220 <- arima(emprego, c(2,2,0))
       > arima221 <- arima(emprego, c(2,2,1))
       > arima221 <- arima(emprego, c(2,2,2))
       > arima221 <- arima(emprego, c(2,2,1))
       ...
       Error in arima(emprego, c(0, 1, 2, 0)) : 
         'order' deve ser um vetor numérico não-negativo de comprimento 3
       > arima0120<- arima(emprego,c(0,1,2))
